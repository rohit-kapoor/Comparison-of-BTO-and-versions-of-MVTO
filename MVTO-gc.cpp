...
// creating a set of functions...
	...
	int read(int id,int array_value,int local_value){
	    ...
	    transaction[dataItems[array_value]->largest_id]->other=id;
	...
	int write(int id,int array_value,int local_value ){
	    ...
	    //dataItems[array_value]->versions.insert(id);
	    scheduler_lock.unlock_shared();
        return 1;        
    }
	...
	void garbageCollect(){
        for(auto&[k,i]:transaction){
	        if(i->get_status()=='s'||(i->other!=-1 && transaction[i->other]->get_status()=='s') )   
		        continue;
...
void udtMem( MVTO_gc* mvGC,int mTid)
{
	...
	for(int i=0;i<g::numTrans;i++)
	{
		...
		do{
			...
			for(int i=0;i<rand;i++)
			{
				...
				mvGC->read(id,rand_index,local_value); // It will read the value at the index rand_index in the local value
			//	printf("value read\n");
				...
				mvGC->write(id,rand_index,local_value);
			...
			status = mvGC->trycommit(id);  // Try to commit the transaction
...
{	int K;
	...
	*/
	...
	std::thread tid[g::n];
	uint64_t res = 0;
	printf("----------Results for MVTO_gc-------------\n");
	for(int i=0;i<g::n;i++){
		g::total_aborts.push_back(0);
		g::total_delay.push_back(0);
	}

	MVTO_gc* mvGC =new MVTO_gc(g::m);

	g::log_file=fopen("MVTO-gc_log_file","w");
	fprintf(g::log_file,"----------Results for MVTO_gc-------------\n");
	for(int i=0;i<g::n;i++){
		tid[i]=thread(udtMem,mvGC,i);
		}	
	for(int i=0;i<g::n;i++){
	    tid[i].join();
    }
