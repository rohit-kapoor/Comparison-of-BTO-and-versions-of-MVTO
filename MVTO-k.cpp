...
class MVTO_k{
	...
	int M;
	int k;
	MVTO_k(int m,int K):M(m){
		k=K;
		counter=0;  // setting the counter initialy to 0 
		dataItems=std::vector<Dataitem*>();           
		for(int i=0;i<M;i++) dataItems.push_back(new Dataitem(Dataitem::initial)); // initialization
...
// creating a set of functions...
	...
	int write(int id,int array_value,int local_value ){
		...
		for (int j : dataItems[array_value]->arr){
		...
		dataItems[array_value]->arr[(dataItems[array_value]->index++)%k]=id;
		scheduler_lock.unlock_shared();
		return 0;
	}
...
void udtMem( MVTO_k* mvK,int mTid)
{
	...
	for(int i=0;i<g::numTrans;i++)
	{
		...
		do{
			...
			for(int i=0;i<rand;i++)
			{
				...
				mvK->read(id,rand_index,local_value); // It will read the value at the index rand_index in the local value
			...
			//	printf("value read\n");
				...
				mvK->write(id,rand_index,local_value);
			...
			status = mvK->trycommit(id);  // Try to commit the transaction
...
{	int K;
    ...
    */
	...
	std::thread tid[g::n];
	uint64_t res = 0;
	printf("----------Results for MVTO_K-------------\n");
	for(int i=0;i<g::n;i++){
		g::total_aborts.push_back(0);
		g::total_delay.push_back(0);
	}

	MVTO_k * mv =new MVTO_k(g::m,K);

	g::log_file=fopen("MVTO_K_log_file","w");
	fprintf(g::log_file,"----------Results for MVTO_K-------------\n");
	for(int i=0;i<g::n;i++){
